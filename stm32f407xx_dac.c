...
***********************************************************************/
void DAC_write(DAC_Handle_t *DACxHandlePtr, uint16_t *digiValPtr)
{
	...
	if(channel == DAC_CHANNEL_1){
		...
		if(resolution == DAC_RES_8_bits){
			DACxHandlePtr->DACxPtr->DHR8R1 = (uint8_t)(*digiValPtr & 0xFF);
		/*case 12 bits DAC resolution*/
		}else{
			uint8_t alignment = DACxHandlePtr->DACxConfigPtr->alignment;
			if(alignment == DAC_ALIGNMENT_LEFT){
				DACxHandlePtr->DACxPtr->DHR12L1 = (*digiValPtr & 0xFFF)<<DAC_DHR12L1_DACC1DHR_Pos;
			}else{
				DACxHandlePtr->DACxPtr->DHR12R1 = *digiValPtr & 0xFFF;
			}
		} 
	}else if(channel == DAC_CHANNEL_2){
		/*case 8 bits DAC resolution*/
		if(resolution == DAC_RES_8_bits){
			DACxHandlePtr->DACxPtr->DHR8R2 = (uint8_t)(*digiValPtr & 0xFF);
		/*case 12 bits DAC resolution*/
		}else{
			uint8_t alignment = DACxHandlePtr->DACxConfigPtr->alignment;
			if(alignment == DAC_ALIGNMENT_LEFT){
				DACxHandlePtr->DACxPtr->DHR12L2 = (*digiValPtr & 0xFFF)<<DAC_DHR12L2_DACC2DHR_Pos;
			}else{
				DACxHandlePtr->DACxPtr->DHR12R2 = *digiValPtr & 0xFFF;
			}
		} 
	}
}
